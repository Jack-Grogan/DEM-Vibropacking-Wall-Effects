...
sliding_pw          = np.round(np.linspace(sliding_pw_min, sliding_pw_max, num_studies),8)
...
for run in study:
    ...
    for seed in seeds:
        ...
        for i in range(num_studies):
            ...
            study_directory = os.path.join(f"{particle_diameter:.0f}_mm_diameter_particles", "sliding_pw", f"sliding_{sliding_pp:.0f}_rolling_{rolling_pp:.0f}", f"seed_{seed}")
            if not os.path.exists(study_directory):
                os.makedirs(study_directory)

            simulation_newpath = os.path.join(study_directory, f"sliding_{sliding_pw[i]:.2f}")
            if not os.path.exists(simulation_newpath):
                os.makedirs(simulation_newpath)

            batch_launch_newpath = os.path.join(study_directory, f"sliding_{sliding_pw[i]:.2f}")
            if not os.path.exists(batch_launch_newpath):
                os.makedirs(batch_launch_newpath)

            mesh_newpath = os.path.join(study_directory, f"sliding_{sliding_pw[i]:.2f}", "mesh")
            if not os.path.exists(mesh_newpath):
                os.makedirs(mesh_newpath)

            inface_newpath = os.path.join(study_directory, f"sliding_{sliding_pw[i]:.2f}", "mesh")
            if not os.path.exists(inface_newpath):
                os.makedirs(inface_newpath)

            toml_path = os.path.join(study_directory, f"sliding_{sliding_pw[i]:.2f}")
